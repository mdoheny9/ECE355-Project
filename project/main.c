...
{ // SOURCE: page 30-31 of SSD1780 Data Sheet, Relevant TIM register bit definitions in stm32f051x8.h
    ...
    snprintf( Buffer, sizeof( Buffer ), "R: %5u Ohms", (uint16_t)PotResistance );
...
void myTIM2_Init() { // does not work!
...
void EXTI2_3_IRQHandler() {
	...
	*/
	...
	if (!display_555 && (EXTI->PR & EXTI_PR_PR2)) {
		//trace_printf("func gen + input detected\n");
		if (firstEdge) {
			firstEdge = 0;
	...
	/* Measure 555 timer signal frequency */
		...
		//trace_printf("555 timer and input detected\n");
		if (firstEdge) {
			//trace_printf("555 first edge\n");
			firstEdge = 0;
			// Clear count register (TIM2->CNT) and start timer (TIM2->CR1).
			TIM2->CNT = 0x00;
			TIM2->CR1 |= TIM_CR1_CEN;
